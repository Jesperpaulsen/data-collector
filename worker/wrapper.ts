...
const createNetworkCallAndTimestampString = (url: string, timestamp: number) => `${url}${timestamp}`;

const createNetworkCallHash = (url: string, timestamp: number) => {
  const urlAndTimestampString = createNetworkCallAndTimestampString(url, timestamp);
  const hash = generateHash(urlAndTimestampString);
  return hash;
};

const storeNetworkCall = (networkCall: NetworkCall) => {
  const networkCallHash = createNetworkCallHash(networkCall.url, networkCall.timestamp);
...
try {
  ...
  chrome.runtime.onMessage.addListener(function (details: { type: string; networkCall: NetworkCall }) {
